    ...
    try {
        ...
        if (position.long) {
            ...
            gains =
                ...
                position.openPrice;
            gainsPercentage =
                ((currentPrice - position.openPrice) / position.openPrice) *
                100;
        } else {
            ...
            gains =
                ...
                position.openPrice;
            gainsPercentage =
                ((position.openPrice - currentPrice) / position.openPrice) *
                100;
        }
...
export const fetchPositionHistory = async () => {
    try {
        const response = await fetch(
            "http://localhost:3001/api/positions/history"
        );
        ...
        console.log("Fetched position history:", history);
        if (!history || !history.length) {
            return [];
        }

        // Parse each historical position using the structure from your backend
        return history.map((pos) => ({
            ...
            long: pos.long,
            matched: pos.matched,
            // Additional fields specific to historical positions
            closedAt: pos.closedAt,
            closedAtBlock: pos.closedAtBlock,
            closeTransaction: pos.closeTransaction,
...
export const calculateProfitLoss = (position, currentPrice) => {
    ...
    if (position.long) {
        ...
        currentValue =
            (position.amount * (2 * position.openPrice - currentPrice)) /
            position.openPrice;
    }
...
export const estimateFees = async (amount) => {
    try {
        ...
        const response = await fetch(
            "https://stacks-node-api.testnet.stacks.co/v2/fees/transfer"
        );
